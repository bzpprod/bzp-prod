...
#==============================================================================#
# Authors: Lars Echterhoff (lars.echterhoff@gmail.com)		 				   #
# Authors: Selman Tunç (stncweb@gmail.com)                                  #
# Copyright © 2010 Lars Echterhoff  -   All Rights Reserved.                   #
#                                                                              #
# Thank you                                                                    #
...
class BrowserDetector
{
	...
	private function detect($pattern, $rxblock = 2)
	{
		...
		foreach($this->pattern[$pattern] as $match_pattern => $qualified)
		{
			...
			{
				$this->{$patvar} = true;
				$this->{$patstr} = (!empty($match[$rxblock]) ? trim($qualified . ' ' . $match[$rxblock]) : trim($qualified));
	...
	function getall()
	{
		foreach($this->pattern as $key => $foo) $this->get($key);
		$this->get('language');
